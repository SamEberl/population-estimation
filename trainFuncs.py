...
def train_reg_from_dino(reg_config, log_dir, ae_model):
    ...
    # Train the model
    for epoch in tqdm(range(reg_config['train_params']['max_epochs'])):
        for i, data in enumerate(train_dataloader):
            inputs, labels = data
            inputs = inputs.to(device)
            labels = labels.to(device)
        ...
        with torch.no_grad():
            for i, data in enumerate(val_dataloader):
                ...
                if i == 0:
                    sample_nbr = random.randint(0, reg_config['data_params']['val_batch_size']-1)
                    log_images_scores_to_tensorboard(writer, step_nbr, sample_nbr, inputs[sample_nbr, :, :, :], labels[sample_nbr], predictions[sample_nbr])
